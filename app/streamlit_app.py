...
with st.sidebar:
    ...
    regions = st.multiselect(
        ...
        default=sorted(customers["region"].dropna().unique())
    )

    policies = st.multiselect(
        "Policy Type",
        sorted(customers["policy_type"].dropna().unique()),
        default=sorted(customers["policy_type"].dropna().unique())
    )

    age_bands = st.multiselect(
        "Customer Age Band",
        sorted(customers["customer_age_band"].dropna().unique()),
        default=sorted(customers["customer_age_band"].dropna().unique())
    )

    risk_categories = st.multiselect(
        "Risk Category",
        sorted(customers["risk_category"].dropna().unique()),
        default=sorted(customers["risk_category"].dropna().unique())
    )
...
filtered_claims = claims[
    claims["customer_id"].isin(filtered["customer_id"])
].copy()
...
high_risk_pct = (
    filtered["risk_category"].eq("High-risk").mean() * 100
...
c1, c2, c3 = st.columns([1.35, 0.9, 1])

# ==========================================================
# CLAIMS VS LOSS SEGMENTATION (FIXED SMALLER BUBBLES)
# ==========================================================
...
# ==========================================================
# CUSTOMER RISK SEGMENTATION (FIXED LABEL VISIBILITY)
# ==========================================================
...
with c3:
    ...
    age_freq = (
        filtered_claims
        .groupby("customer_age_band", as_index=False)
